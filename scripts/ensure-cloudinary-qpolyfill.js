...
const targetPath = path.join(__dirname, '..', 'node_modules', 'cloudinary', 'lib', 'utils', 'qPolyfill.js');

const fallbackContent = `const scheduleCompatCallback = typeof setImmediate === 'function'
  ? (fn) => setImmediate(fn)
  : (fn) => setTimeout(fn, 0);
...
try {
  if (!fs.existsSync(targetPath)) {
    ...
    console.log('Added missing Cloudinary qPolyfill.js');
  }
} catch (err) {
  console.warn('Failed to ensure Cloudinary qPolyfill.js:', err);
}
